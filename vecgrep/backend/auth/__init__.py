...
Division of labor with the MCP SDK (mcp.server.auth):
  ...
  - The approval middleware proves owner presence before /authorize reaches the
    provider; dynamic client registration alone authenticates no human.
...
Scope model: tokens carry scopes. `read` = search/list/get; `propose` = every
write-shaped tool, including the deployment-opted direct-write surface. There
is NO `confirm` scope — confirmation stays a human action off the OAuth protocol
entirely (the wall).
"""
