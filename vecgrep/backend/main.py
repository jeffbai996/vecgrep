...
<footer><span>OAuth 2.1 · PKCE</span><span>approval lasts a year</span></footer>
    ...
    if FRONTEND_DIR.is_dir():
        ...
        # One route per icon, named explicitly rather than a /{path} catch-all,
        # which would swallow every unmatched GET in the app. Only /assets is
        # mounted, so a file at the root otherwise falls through to the SPA
        # catch-all and the browser is handed a page where it wanted an image.
        #
        # Three of them because one is not enough: Safari ignores an SVG
        # apple-touch-icon, and a browser that falls back to /favicon.ico was
        # being served index.html (2026-09-09).
        def _icon_route(name: str, media: str) -> None:
            @app.get(f"/{name}", include_in_schema=False, name=f"icon_{name}")
            def _serve() -> FileResponse:
                return FileResponse(FRONTEND_DIR / name, media_type=media)

        for _name, _media in (("favicon.svg", "image/svg+xml"),
                              ("favicon.ico", "image/x-icon"),
                              ("apple-touch-icon.png", "image/png")):
            _icon_route(_name, _media)
